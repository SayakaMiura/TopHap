...
if os.path.exists('Bootstrap')==True:
 ...
 for FasLs in FasTaLs:
    ...
    for i in FasLs:
            ...
            for i in StLs:
                       ...
                       Seq2OriID[St2Seq[i]]=Seq2OriID.get(St2Seq[i],[])+[i]	
 ...
 for Hap in TopHap2Data:
    ...
    OriSeqLs=Seq2OriID.get(Hap,[])
    for OriSeqID in OriSeqLs:
        outAnno+=ID+'_'+TopHap2Data[Hap][0].replace(' ','_')+'_'+str(len(TopHap2Data[Hap]))+'\t'+OriSeqID.replace('>','')+'\n'
        
    TopHap2ID[Hap]=ID	
    ID2TopHap[ID]=Hap	
    TopID+=1
 Functions3.GetOut('TopHap.txt',outAnno)   
 ...
 os.system('megacc -a infer_NJ_nucleotide.mao -d ' + 'TopHap.fasta' +' -o ' + 'TopHap_NJ.nwk')
